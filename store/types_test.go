...
func TestParseNodeType(t *testing.T) {
	tests := []struct {
		...
		{in: "custom-32767", want: CustomNodeType(32767)},
		{in: "130", want: NodeType(130)},
		{in: "65535", want: NodeType(65535)},
		{in: "custom:40000", wantErr: true},
...
func TestParseEdgeType(t *testing.T) {
	tests := []struct {
		...
		{in: "custom:32767", want: CustomEdgeType(32767)},
		{in: "65535", want: EdgeType(65535)},
		{in: "custom:40000", wantErr: true},
		{in: "65536", wantErr: true},
		{in: "", wantErr: true},
